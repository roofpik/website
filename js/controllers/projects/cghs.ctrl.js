app.controller('cghsCtrl', ['$scope', '$http', '$timeout', '$stateParams', '$state', '$interval', function($scope, $http, $timeout, $stateParams, $state, $interval) {
    ...
    function fetchProjects() {
        $http({
            url: 'http://107.23.243.89/api/GetCghs_1.0',
            method: 'GET',
            params: {
    ...
    $scope.applyFilters = function() {
        ...
        if($scope.minPrice > $scope.maxPrice){
            swal("Error!", "Minimum price entered is greater than maximum price entered");
        }
        if($scope.minArea > $scope.maxArea){
            swal("Error!", "Minimum area entered is greater than maximum area entered");
        }
        if($scope.minArea > $scope.maxArea && $scope.minPrice > $scope.maxPrice){
            swal("Error!", "Check the area and price you've entered");
        }
    ...
    $state.go('cghs', { bhk: $scope.bhk, price_range: $scope.priceRange, area_range: $scope.areaRange, location: $scope.location, locality: $scope.locality, propertyType: $scope.type });
}
